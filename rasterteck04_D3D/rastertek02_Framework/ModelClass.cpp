...
bool ModelClass::InitializeBuffer(ID3D11Device* device)
{
	...
	vertices[0].color = XMFLOAT4(0.3f, 0.9f, 0.2f, 1.0f);

	vertices[1].position = XMFLOAT3(0.0f, 0.5f, 0.0f);
	vertices[1].color = XMFLOAT4(0.05f, 1.0f, 0.6f, 1.0f);

	vertices[2].position = XMFLOAT3(0.5f, -0.5f, 0.0f);
	vertices[2].color = XMFLOAT4(0.0f, 0.8f, 0.5f, 1.0f);

	indices[0] = 0;
	indices[1] = 1;
	indices[2] = 2;

	// static vertex buffer desc 설정
